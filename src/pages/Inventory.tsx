...
const Inventory = () => {
  ...
  const resetForm = () => {
    setNewProduct({ name: "", sku: "", price: 0, cost: 0, stock_quantity: 0, category_id: "", imageFile: null });
    setFormErr(null);
  };
  ...
  const handleFile = (file: File) => {
    if (!file.type.startsWith("image/")) {
      setFormErr("El archivo debe ser una imagen");
      return;
    }
    if (file.size > MAX_IMG_SIZE) {
      setFormErr("La imagen supera 2 MB");
      return;
    }
    setFormErr(null);
  ...
  const validate = () => {
    ...
    if (newProduct.price <= 0) return "Precio debe ser mayor que 0";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 pb-20">
      ...
      {/* Header */}
      {/* omito estadísticas para brevedad … */}
        ...
        {isSuperAdmin && (
          <Dialog open={isAddOpen} onOpenChange={(o) => { setIsAddOpen(o); resetForm(); }}>
            <DialogTrigger asChild>
              <Button><Plus className="w-4 h-4 mr-1" />Agregar producto</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-3" onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
                ...
                {/* Área drag & drop */}
                <div className="border-2 border-dashed p-4 rounded text-center cursor-pointer"
                  ...
                  {newProduct.imageFile ? (
                    ...
                    <p className="text-gray-500 flex flex-col items-center">
                      ...
                      Suelta o haz clic para subir imagen (máx 2 MB)
                    </p>
                  )}
      ...
      {/* Tabla … (sin cambios en lógica principal, solo lectura) */}
      {/* … */}
    </div>
  );
};
